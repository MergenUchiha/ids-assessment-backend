...
function fakeRunDates(status) {
    if (status === 'QUEUED')
        return { startedAt: null, finishedAt: null };
    const start = faker_1.faker.date.recent({ days: 30 });
    if (status === 'RUNNING')
        return { startedAt: start, finishedAt: null };
...
async function main() {
    ...
    const extraUsers = await Promise.all(Array.from({ length: 3 }, async () => {
        const hash = await bcrypt.hash('password123', 10);
        return prisma.user.create({
            data: { email: faker_1.faker.internet.email().toLowerCase(), password: hash },
        });
    }));
    console.log(`   ✓ ${1 + extraUsers.length} users`);
    console.log('🛡  Creating IDS profiles...');
    const profiles = await Promise.all(RULESETS.map((ruleset, i) => prisma.idsProfile.upsert({
    ...
    const scenarios = await Promise.all(MSF_MODULES.map(({ module, rport, sigs }) => {
        ...
        return prisma.scenario.create({
            data: {
                name: shortName,
                description: faker_1.faker.hacker.phrase(),
                msfModule: module,
    ...
    const EXPERIMENT_CONFIGS = [
        {
            ...
            description: 'Initial calibration run — default Suricata rules against common scanners',
            runsCount: 8,
        },
        {
            name: 'Emerging Threats Benchmark',
            description: 'Testing emerging-threats-open ruleset against 0-day simulations',
            runsCount: 6,
        },
        {
            name: 'EternalBlue Detection Study',
            description: 'Focus on MS17-010 detection rate across different rule configurations',
            runsCount: 5,
        },
        {
            name: 'Web App Attack Coverage',
            description: 'SQL injection, directory traversal and RCE scenarios against web targets',
            runsCount: 7,
        },
        {
            name: 'Network Scan Detection',
            description: 'Evaluating scan detection: Nmap, SYN, UDP and service probes',
            runsCount: 10,
        },
        {
    ...
    for (const cfg of EXPERIMENT_CONFIGS) {
        ...
        for (let i = 0; i < cfg.runsCount; i++) {
            ...
            const attackSuccess = status === 'FINISHED'
                ? faker_1.faker.datatype.boolean({ probability: 0.55 })
                : null;
                    ...
                    if (status !== 'FAILED') {
                        ...
                        await prisma.attackEvent.create({
                            data: {
                                runId: run.id,
                                type: evType,
                                timestamp: new Date(finishedAt.getTime() - randomInt(500, 3000)),
                                data: {
            ...
            if (status === 'FINISHED' || status === 'RUNNING') {
                ...
                const expectedSigs = scenario.expectedSignatures;
                const alertPool = [
                    ...pickMultiple(SURICATA_SIGNATURES, randomInt(2, 5)),
                    ...expectedSigs.map((s) => ({ sig: s, sev: 1 })),
                ];
                for (let a = 0; a < alertCount; a++) {
                    const sigEntry = randomFrom(alertPool.length > 0 ? alertPool : SURICATA_SIGNATURES);
                ...
                if (status === 'FINISHED') {
                    ...
                    const precision = tp + fp > 0
                        ? tp / (tp + fp)
                        : Math.random() > 0.5
                            ? parseFloat(faker_1.faker.number
